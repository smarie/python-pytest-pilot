...
if [ "${TRAVIS_PYTHON_VERSION}" = "3.5" ]; then
   ...
   echo -e "\n\n****** Running tests ******\n\n"
   coverage run --source pytest_pilot -m pytest --junitxml=reports/junit/junit.xml --html=reports/junit/report.html -v pytest_pilot/tests/
   # buggy
   # python -m pytest --junitxml=reports/junit/junit.xml --html=reports/junit/report.html --cov-report term-missing --cov=./pytest_pilot -v pytest_pilot/tests/
else
